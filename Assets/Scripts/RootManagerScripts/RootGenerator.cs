...
{
    public void StartGrowingRoot(List<Vector2> rootPoints, int manaConsumed, FertileTerrain fertileTerrain)
    {
        Root root = Instantiate(Resources.Load("Root") as GameObject, Vector3.zero, Quaternion.identity).GetComponent<Root>();
        root.StartGrowing(rootPoints, manaConsumed, fertileTerrain);
    }
